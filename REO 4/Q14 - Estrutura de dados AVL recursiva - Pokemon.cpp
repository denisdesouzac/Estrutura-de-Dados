...
noh* avl::insereAux(noh* umNoh, const pokemon& umDado) {
    //#WARNING implemente
    if(umNoh == NULL){
        noh* novo = new noh(umDado);// Cria um nó com o dado.
        return novo;
    }else{
...
noh* avl::arrumaBalanceamento(noh* umNoh) {
    ...
    atualizaAltura(umNoh);// atualiza a altura de umNoh

    int fatorBal = umNoh->fatorBalanceamento();// Checa se está balanceado
