...
class campeonato {
    ...
    public:
        ...
        campeonato(const campeonato &segundo);
        ~campeonato();
...
campeonato::campeonato(const campeonato &secundario) {// NOVO Construtor

    cout << endl <<"construtor de copia" << endl << endl;
    
    quantidadeDeCompetidores = secundario.quantidadeDeCompetidores;
    capacidadeMaxima = secundario.capacidadeMaxima;
    listaDeCompetidores = new competidor[capacidadeMaxima];
    
    for (int i = 0; i < capacidadeMaxima; i++) {
        listaDeCompetidores[i] = secundario.listaDeCompetidores[i];
    }
...
void campeonato::redimensionarCapacidade() { // Método implementado

    competidor *vetorAux = new competidor[capacidadeMaxima];
    for(int i = 0; i < capacidadeMaxima; i++){
        vetorAux[i] = listaDeCompetidores[i];
    }

    delete[]listaDeCompetidores;

    int capacidadeMaximaAntiga = capacidadeMaxima;
    capacidadeMaxima = capacidadeMaxima + 4;// A capacidade sorteada é aumetar 4;
    listaDeCompetidores = new competidor[capacidadeMaxima];

    for(int i = 0; i < capacidadeMaximaAntiga; i++){
        listaDeCompetidores[i] = vetorAux[i];
    }

    delete[]vetorAux;

    cout << endl << "A nova capacidade eh : " << capacidadeMaxima << endl << endl;
}
